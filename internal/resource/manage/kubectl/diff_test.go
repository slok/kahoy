	...
	tests := map[string]struct {
		...
		"Having resources should apply correctly.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"kubectl", "diff", "--force-conflicts=true", "--server-side=true", "-f", "-"},
					"test",
				)
		...
		"Having custom command should apply with the specific cmd.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"whatever", "diff", "--force-conflicts=true", "--server-side=true", "-f", "-"},
					"test",
				)
				mc.On("Run", mock.MatchedBy(exp)).Once().Return(nil)
			},
		...
		"Having kubectl context set, should set the cmd flag.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"kubectl", "diff", "--context", "whatever", "--force-conflicts=true", "--server-side=true", "-f", "-"},
					"test",
				)
				mc.On("Run", mock.MatchedBy(exp)).Once().Return(nil)
			},
		...
		"Having kubectl config set, should set the cmd flag.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"kubectl", "diff", "--kubeconfig", "whatever", "--force-conflicts=true", "--server-side=true", "-f", "-"},
					"test",
				)
				mc.On("Run", mock.MatchedBy(exp)).Once().Return(nil)
			},
		...
		"Having force disabled, should set the cmd flag.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"kubectl", "diff", "--force-conflicts=false", "--server-side=true", "-f", "-"},
					"test",
				)
				mc.On("Run", mock.MatchedBy(exp)).Once().Return(nil)
			},
		...
		"Having field manager set, should set the cmd flag.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner) {
				...
				exp := expCmdMatcher(
					[]string{"kubectl", "diff", "--force-conflicts=true", "--field-manager", "whatever", "--server-side=true", "-f", "-"},
					"test",
				)
				mc.On("Run", mock.MatchedBy(exp)).Once().Return(nil)
			},
...
func TestDiffManagerDelete(t *testing.T) {
	tests := map[string]struct {
		config    kubectl.DiffManagerConfig
		resources []model.Resource
		mock      func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner, mfs *kubectlmock.FSManager)
		expErr    bool
	}{
		"Not having resources, shouldn't execute anything.": {
			resources: []model.Resource{},
		...
		"Having an error while encoding resource should stop execution and fail.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner, mfs *kubectlmock.FSManager) {
				...
				mk.On("EncodeObjects", mock.Anything, mock.Anything).Once().Return(nil, fmt.Errorf("whatever"))
			},
			expErr: true,
		},
		...
		"Having an error while storing encoded resource should stop execution and fail.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner, mfs *kubectlmock.FSManager) {
				...
				mk.On("EncodeObjects", mock.Anything, mock.Anything).Once().Return([]byte("test1"), nil)
		...
		"Having resources should apply correctly.": {
			...
			mock: func(mk *kubectlmock.K8sObjectEncoder, mc *kubectlmock.CmdRunner, mfs *kubectlmock.FSManager) {
				...
				expK8sResources1 := []model.K8sObject{newK8sObject("test1", "ns1")}
				mk.On("EncodeObjects", mock.Anything, expK8sResources1).Once().Return([]byte("test1"), nil)
				...
				expK8sResources2 := []model.K8sObject{newK8sObject("test2", "ns2")}
				mk.On("EncodeObjects", mock.Anything, expK8sResources2).Once().Return([]byte("test2"), nil)
